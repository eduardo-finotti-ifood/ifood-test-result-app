...
import { StyleSheet, Text, View, FlatList, Image, ImageBackground, TouchableOpacity, StatusBar } from 'react-native';
...
export default class IOSDailyScreen extends Component {
  ...
  componentDidMount() {
    axios.get('http://192.168.169.105:8000/api/results?run=daily&platform=ios&quantity=10')
    .then(response => {
    ...
    return (
      <View style={styles.container}>
        ...
        <SafeAreaView  style={{backgroundColor: '#1f1f1f', height: 75}}>
          <View style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start', paddingTop: 30}}>
            <Icon style={{color: 'white', paddingLeft: 20}} name='menu' onPress={() => this.props.navigation.openDrawer()} />
            <Text style={{color: 'white', fontSize: 20, paddingLeft: '30%'}}>Daily iOS</Text>
          </View>
        </SafeAreaView>
        ...
        <View style={{ alignItems: 'flex-start', justifyContent: 'center',}}>
          ...
          <View style={{ padding: 10}}>
            <Text style={{color: 'white', fontSize: 25, padding:10}}>Daily Run for iOS</Text>
          </View>
